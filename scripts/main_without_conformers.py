...
number_of_data = 50 #57462 #20000 #20000
number_of_data_COMP = 50 #2999 #20000
number_of_seeds = 10
...
print(X.shape)
train_test_ratio = 0.15 #0.00025
final_train_test_ratio = 0.66 #0.015
...
compute_learning_curve(ratios, number_of_data, proportion, mol_indices, np.copy(Y), np.copy(X_feature_matrix), np.copy(X_benchmarkCOMP), np.copy(Y_benchmarkCOMP), np.copy(X_benchmarkANI), np.copy(Y_benchmarkANI),method_list, number_of_seeds, restart=True)
